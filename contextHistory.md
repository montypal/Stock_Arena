...
Every league has a fixed starting balance (Monday 7:00 AM start, Sunday 7:00 PM end), a player group, a leaderboard, real prices, fake money, shared achievements, and a winner. You can join and trade anytime that window is open — later just means less time to grow.
...
Jackson OpenCode 11:45 PM 9/23/26 PDT

Fixed the leagues system at Jackson's request: every league now opens **Monday 7:00 AM ET → Sunday 7:00 PM ET**, any of the three tiers can be joined at any point while the window is open (later = less time to grow), and a player can be in 1K + 10K + 100K at once but never the same tier twice. Pulled `origin/main` first (`9a2db3d`); no collaborator commits to merge; tree clean except untracked `web/package-lock.json` (left untracked); no destructive git ops; no secrets.

* **Timing — 6 AM → 7 AM ET:** `web/lib/trading/game.js` — join insert `time '06:00'` → `time '07:00'` and `JOINABLE_WEEK` comment updated; `weekHasStarted(ws)` now checks `Monday 07:00 ET` (not Monday midnight) so TierCards' "Live now" matches the real open; `worker/schema.sql` comment `Monday 06:00` → `07:00` plus an idempotent `UPDATE leagues SET starts_at = Monday 07:00` for any still-open leagues created under the old 06:00 window (settled leagues untouched). Verified: no remaining `06:00` in code except that migration's `WHERE` clause.
* **Docs — current plan only, no history rewritten:** `README.md` intro + `## How a league week works` bullets `Monday 6:00 AM` → `7:00 AM`; `contextHistory.md` Regular Gameplay Plan `## Weekly Game Loop` Monday line, `## League Structure`, and `## Trading` (`Monday 6:00 AM ET until Sunday 7:00 PM`) → `7:00 AM ET`; `web/components/battle/StandingCard.js` — `Starts Monday, Sep …` → `Starts Monday Sep … 7:00 AM ET · orders fill at the open` and `Trading closed · final results Monday 12:00 AM ET` → `Trading closed · final results after Sunday 7:00 PM ET` (settlement is on `ends_at = Sunday 19:00 ET`, not Monday midnight).
* **One-per-tier, all three allowed — UI now respects it:** DB already had `leagues UNIQUE(tier, week_start)` and `entries UNIQUE(user_id, league_id)` plus `joinLeague`'s per-tier check; the bug was `web/app/(battles)/league/page.js` and `web/app/page.js` hiding all join cards once the player had ANY entry this week. Fixed by switching both pages from `currentEntry()` to `entriesForWeek(user.id, ws)` (added `entriesForWeek ORDER BY l.tier` + new helper `entryForTier()` in `game.js`). `web/components/battle/TierCards.js` gains optional `tiers`/`joinedTiers` props — joined tiers render as "Joined · Sep …" instead of a second Join button, remaining tiers still show a working Join. Home and Battles show a compact tier switcher (`?tier=1000|10000|100000`) when the player has 2–3 entries this week; the selected entry's StandingCard/Holdings/PendingOrders/Leaderboard render, invalid `?tier=` falls back to lowest tier.
* **Trading into the right league:** `web/app/(battles)/trade/[symbol]/page.js` and `web/app/(battles)/trade/page.js` now fetch the same `entriesForWeek` set and offer the same `?tier=` picker when `entries.length > 1`; the detail page's buy/sell forms include hidden `entry_id` + `tier` so `web/lib/actions.js:trade()` can call `placeOrder(..., { entryId })` (already supported by `game.js:placeOrder`) and redirect back to `?tier=` of that league. `web/components/trade/StockList.js` carries `?tier=` through to the detail page. `web/app/(battles)/trade/page.js` bug-fixed — the previous `list.length` reference inside `TradePage` (where `list` was only in `StockResults`) removed; the toolbar now renders without it.
* **Verified:** `cmd /c npm run build` in `web/` green (Next 15.5.25, `ƒ /`, `ƒ /league`, `ƒ /trade`, `ƒ /trade/[symbol]`, 5/5); `python -m unittest test_game` 6/6 in `worker/` + `py_compile` clean; no secrets committed; no bots/fake players added (rule 17).
* **Follow-up:** Railway worker will apply the `starts_at` UPDATE on next restart; join/switcher/trade flows to be smoke-tested on Vercel after deploy; if Home's "Joined" pill wants richer styling, adjust `web/styles/screens/*.css` without touching league logic.

---

Aarav — Claude Code 6:13 AM 9/25/26 PDT

Fixed the home-page server exception and the reason no deploy had landed since `6a5f9c1`. Aarav reported "Application error: a server-side exception" (digest 2123131494) on the home page only, plus the header 3D runner missing and `/models/retargeted_animations.glb` 404 on Vercel. Pulled `origin/main` first — local was **6 commits behind** (`97b5ee4` → `f6ec2a0`), fast-forward only, tree clean, no collaborator work touched, no destructive git ops, no secrets, no bots/fake players (rule 17).

* **Why the home page crashed:** `web/app/page.js` called `holdings(entry.id)` inside `HomeDetail` but never imported `holdings` from `../lib/trading/game` (it is exported there at line 213; `/trade/[symbol]` and `/profile` import it correctly). That is a `ReferenceError` during the Server Components render. It only fires for a signed-in player who has joined a league — the only path that reaches `HomeDetail` — which is why signed-out visitors and `/league` were fine. Introduced in `f2f3aad`, so it had been live ever since. Fixed by adding `holdings` to the existing import.
* **Why nothing deployed since `6a5f9c1` (the GLB 404):** that commit introduced two import specifiers with the wrong depth — `web/app/(battles)/league/page.js` used `'../../components/home/PortfolioSummary'` (resolves to `web/app/components/...`, which does not exist; every sibling import in that file uses `'../../../'`), and `web/app/(battles)/trade/[symbol]/page.js` used `'../../../../components/ui'` (no such module; `Flash` lives in `components/layout/ui.js`). Either one fails `next build` with "Module not found", so Vercel rejected the whole deployment and production stayed on the older build — which is exactly why `/models/running.fbx` and `/models/textures/packed/Image_0.png` still served 200 while the newer `retargeted_animations.glb` 404'd. The GLB itself was never the problem: it is committed intact (10,043,916 bytes, valid `glTF` magic, not an LFS pointer, nothing ignoring it), and `public/` serving works. Both specifiers corrected.
* **Third bug found while auditing:** `web/app/(battles)/trade/[symbol]/page.js` called `summarize(entry, rows)` unconditionally while `entry` is legitimately `null` (lines 36-38 already guard it), and `summarize()` dereferences `entry.cash` — a crash on any stock page for a signed-in player who has not joined a league this week, reachable in one click from `/trade`. Guarded both the call and the `<PortfolioSummary>` render.
* **Runner background removed (requested):** the `<Canvas>` was already transparent (`gl={{ alpha: true }}`, `style={{ background: 'transparent' }}`, no scene background/fog/Environment). The visible backdrop was CSS — `.runner-fly` and `.runner-fly-static` in `web/app/globals.css` each painted a gold/green radial+linear gradient pill behind the canvas. Both are now `background: none`; the poster fallback and all animation/framing code are untouched.
* **`running.fbx` deleted** (`git rm web/public/models/running.fbx`, 6,340,128 bytes) — nothing references it and there is no FBX loader in the code; `useGLTF` loads the GLB only.
* **Verified:** no Node on this machine, so no `next build` here — Vercel's build is the first compile, as in earlier entries. Gates used instead: (1) a case-sensitive resolver over all **120** relative import specifiers in `web/` (Vercel builds on Linux, where a wrong-case path fails even though Windows tolerates it) — all resolve; (2) confirmed `Flash` is exported from `components/layout/ui.js`, `PortfolioSummary` has a default export, and `holdings` is exported from `lib/trading/game.js`; (3) three parallel read-only audits (missing imports/undefined identifiers, SSR + build hazards, asset-reference integrity) across all 49 `.js` files — the three bugs above were the only real findings; (4) live probes showed `/`, `/league`, `/trade`, `/daily`, `/profile`, `/progress`, `/status`, `/models/running.fbx`, `/models/textures/packed/Image_0.png`, `/logo-mark.png` all 200 and only the GLB 404. Note `/leagues` (plural) does not exist — the route is `/league`.
* **Follow-up:** after this deploy, confirm `/models/retargeted_animations.glb` returns 200 (~10 MB) and the header shows a running bull on a transparent backdrop, and re-check the home page while signed in with a league joined. `web/package-lock.json` is still untracked, so Vercel re-resolves `@react-three/drei ^10`, `fiber ^9` and `three ^0.186` on every build — committing a lockfile remains the way to stop local and Vercel installs drifting (already noted in the 9/23 entry).

---

Aarav — Claude Code 6:31 AM 9/25/26 PDT

Made the header runner actually draw. After the previous entry's deploy landed, the GLB served 200 (10,043,916 bytes), `running.fbx` 404'd and the CSS backdrop was gone — but the header showed nothing at all, because the character was never rendering. Two separate defects in `web/components/layout/HeaderRunner.js`, both pre-existing and both hidden until the gold/green pill stopped covering the empty canvas.

* **Zero draw calls (the character was frustum-culled):** instrumented the live page by wrapping `WebGL(2)RenderingContext.prototype.drawElements/drawArrays/…/clear` — over 26 pumped frames three.js issued **26 clears and 0 draws**, so the render loop was healthy and the mesh simply never entered the render list. It is a single skinned mesh (`skins: 1`), and three.js culls skinned meshes against a bounding volume taken from the bind pose; the console's own `posed bounds` log shows that volume centred at **y ≈ 2.65**, far outside a camera sitting at `[0, 0.62, 2.15]`. The auto-fit moves the group into frame but does not refresh that volume, so the mesh tested as off-screen forever. Fixed with a `gltf.scene.traverse(o => { if (o.isMesh) o.frustumCulled = false; })` effect — one small mesh is cheaper to always draw than to re-measure each frame. No change to the fit, camera, lights, or materials.
* **Wrong clip — it was playing the backflip:** the GLB carries five clips and the code played `gltf.animations[0]`. Parsing the GLB's JSON chunk locally gives the real order: **`Backflip`, `Jog`, `Pushup`, `Run_Anime`, `Sprint`** — so index 0 is a backflip, not a run, which the console confirmed (`playing "Backflip"`). Clip order is an export artefact, so the clip is now picked by name (`/^run/i` → `Run_Anime`, then any `run|sprint|jog`, else index 0), and the pelvis root-motion pinning now targets the clip that actually plays instead of always `animations[0]`.
* **Verified:** still no Node on this machine, so no `next build` here — Vercel's build is the first compile. Before editing: tree clean, on `main`, level with `origin/main`, no collaborator work touched, no destructive git ops, no secrets, no bots/fake players (rule 17). Diagnosis was done entirely against the live deployment: `fitted-ready` reached, `auto-fit scale 1.39793`, no diag errors, canvas present at 56×36 with `alpha: true` and `background: none` on `.runner-fly`, poster correctly swapped out once ready. Note the Browser pane here is hidden, so `requestAnimationFrame` does not fire on its own — frames had to be pumped with screenshots; that is a harness artefact, not a site bug.
* **Follow-up:** after this deploy, re-check that draw calls are non-zero and the bull is visible mid-header. The home page still needs one pass while signed in with a league joined — that path depends on a per-browser device hash and cannot be exercised from here. `web/package-lock.json` remains untracked (see the 9/23 and 6:13 AM entries).

---

Jackson OpenCode 9/25/26 PDT

Brightened the header 3D runner at Jackson's request ("the model appears too dark, I want to be able to see it in detail"). Pulled `origin/main` first — local `f6ec2a0` was behind; the pull aborted because a local untracked copy of `web/public/models/running.fbx` (6,340,128 bytes) collided with the incoming deletion of that file, so it was restored from HEAD (`git restore --source=HEAD --staged --worktree`) to let the merge apply its own deletion cleanly. Fast-forwarded to `0ebb01d`; no collaborator work overwritten, no destructive git ops, no secrets.

* **Why it was dark (not the model, not the texture):** the light rig was written for three.js **legacy lighting**, which was removed in r155 — `three` is at `^0.186.0` and `@react-three/fiber` is `^9.7.0`, so the renderer is in physical units and the old `ambientLight 1.15` / `directionalLight 1.6` / fill `0.45` rig was effectively unlit, leaving the character a near-black silhouette against the dark header glass. R3F also defaults to ACESFilmic tone mapping, whose midtone rolloff darkened what little light there was.
* **Fix — `web/components/layout/HeaderRunner.js`, lights only:** `ambientLight` 1.15 → **2.6**; added a `hemisphereLight` (`#ffffff` sky / `#93a7c4` ground, **1.9**) for overall readability; key `directionalLight` 1.6 → **3.4**; left fill 0.45 → **1.5**; added a front light at `[0.4, 1.2, 3]` (**1.8**) aimed at the camera so the face/chest catch detail instead of reading as a silhouette. Also set `state.gl.toneMappingExposure = 1.5` in the existing `onCreated` and logged it with the canvas-created line, so the exposure is visible in the existing diagnostics.
* **Not changed:** model, texture, camera `[0, 0.62, 2.15]`/fov 35, `FIT_H 1.175`, `frustumCulled = false` fix, run-clip selection, pelvis root-motion pinning, DOM traversal loop, poster fallback, CSS. No league/trading/worker/schema changes.
* **Verified:** `cmd /c npm run build` in `web/` green (Next 15.5.25, 5/5 static, all routes unchanged). Brightness itself was **not** verified by screenshot on this machine — `requestAnimationFrame` does not fire in the headless Browser pane here (same harness limit Aarav hit at 6:31 AM), and the live site only shows this after Vercel deploys. The values are derived from the r155+ lighting change, not from a measured before/after render.
* **Follow-up:** confirm on the deployed site that the runner is bright and not washed out; if it is still dim, the next lever is the GLB's own material (a `MeshStandardMaterial` with a dark baked baseColor needs either higher `emissive` or a brighter texture), and if it is now too bright, drop `toneMappingExposure` and the front light first. `web/package-lock.json` remains untracked.

---

Jackson OpenCode 9/25/26 PDT

Header runner: removed the image that showed before the 3D bull, slowed him down, and made him bigger. Pulled `origin/main` first (level at `001a78b`, the lighting commit); tree clean except untracked `.hermes/` and `web/package-lock.json` (both left untracked); no destructive git ops, no secrets, no bots/fake players (rule 17).

* **The "image of the bull" (reported bug):** it was never a fallback image of the render — `HeaderRunner.js` rendered `<img src="/models/textures/packed/Image_0.png" class="runner-poster">` for the whole time `ready` was false, i.e. the raw texture bitmap of the model, travelling the loop until the GLB finished loading. Deleted the `<img>`, the `ready` state, `LOCAL_TEXTURE`, the texture `HEAD` probe, the `.runner-fly .runner-poster` rule, and the `texture` guidance + `texture-applied` stage in `RunnerDiag.js` (both became unreachable). Also deleted `.runner-fly-static` and its `runner-across` keyframes from `globals.css` — dead since `header.js` moved to `dynamic(..., { loading: () => null })`, so that CSS-only poster flyer was no longer rendered by anything. The header now shows the real GLB or nothing: no stand-in image on any path, including WebGL failure and the error boundary.
* **Nothing crosses the header before the bull exists:** the rAF traversal now waits on a `readyRef` that flips on `fitted-ready`, so the empty canvas no longer slides across first and the bull enters at the left edge instead of popping in mid-track. The gate is load-bearing on the `start` assignment order (see the comment at the guard).
* **Slower:** clip `timeScale` 0.75 → **0.6** (`ANIM_RATE`) and crossing 2600 → **3250 ms** (`CROSS_MS`), both ~0.8x. They have to move together — slowing only the stride rate reads as a moonwalk. `Run_Anime` is 0.75 s, so a cycle is 1.25 s at 0.6x and the bull takes ~2.6 strides per crossing. `GAP_MS` 2000 unchanged.
* **Bigger:** `.runner-track` 36 → **44px**, `.runner-fly` 56x36 → **64x44** (canvas 54 → 66 device px at dpr 1.5, +49% pixels for surface detail). `FIT_H` retuned 1.175 → **1.05** — see below for why it went *down* while the bull got bigger.
* **A real clipping bug found while sizing this:** the auto-fit normalises to a single posed frame (frame 12), but the run cycle's full vertical extent is ~1.2x that frame, so at the old settings the character was ~44 CSS px of content inside a 36px canvas — roughly **8 CSS px of legs/hooves were cut off at the bottom**, which is likely part of why he read as a dark blob. Measured the true relationship by rendering at a deliberately small `FIT_H` (0.9, where nothing can touch an edge) and sampling 14 frames across the 1.25 s cycle: rendered content = **0.833 * FIT_H** of the canvas height. Added `V_BIAS = 0.048 * FIT_H` upward in the fit, because the lowest footfall drops further below the fitted pose than the head rises above it and the content hung ~2.5px low in the frame.
* **Verified by measurement, not by eye** (this model cannot view images, so every claim below is a pixel or API reading): Playwright/Chromium against `next dev` on `/status` — `fitted-ready` reached, **0 console errors**, **0 runner-diag errors**, WebGL2 available, `/models/retargeted_animations.glb` 200 `model/gltf-binary`; `img.runner-poster` count **0** document-wide, **0** `img` inside the flyer, the only header `img` is `/logo-mark.png`; `drawElements` ~**59/s** (non-zero — the original zero-draw regression); flyer transform held at the CSS default until `fitted-ready`, then advanced to 1039px = trackW 959 + OFF 80 and parked for the gap; `prefers-reduced-motion` parks at (959-64)/2 = 447.5px with no traversal; 24 frames (~2.3 cycles) at the final config give union y 3..60 of 0..65 → **2 CSS px top and 3.3 CSS px bottom clearance**, x margins 20/27px, no pixel touching any edge, full character **38.7 CSS px** tall (old build: 36 CSS px visible, legs cut). `cmd /c npm run build` green.
* **Local-env note, not a regression:** `/` cannot render on this machine — `app/page.js` → `joinableWeek` → `lib/db/index.js:26` `pool.query` throws `ECONNREFUSED` (no local Postgres), which tears the tree down before the header mounts. Verification used `/status`, which renders the same header with no DB. Pre-existing and unrelated to the runner; worth a local Postgres or a `SKIP_DB` path if header work needs the home page again.
* **Follow-up:** `FIT_H` above ~1.10 in a 44px box re-introduces edge clipping, and the box cannot grow much past 44px inside the 68px header — a genuinely larger bull needs a taller header or a camera/framing change, not a bigger constant. The 10 MB GLB means the header is empty for the first seconds on a cold cache; that is now visible as *nothing* rather than a texture bitmap, which is what was asked for.
